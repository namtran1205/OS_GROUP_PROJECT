...
BootSector::BootSector(shared_ptr<SectorReader> sectorReader)
{
    ...
    this->numberEntriesOfRDET = Utils::MyINTEGER::Convert2LitleEndian(bootSector.begin() + 0x11, 4);
    this->sectorPerFat = Utils::MyINTEGER::Convert2LitleEndian(bootSector.begin() + 0x24, 4);
    this->sectorPerCluster = (uint8_t)(*(bootSector.begin() + 0xD));
    this->sectorPerBootsector = Utils::MyINTEGER::Convert2LitleEndian(bootSector.begin() + 0xE, 2);
    this->sectorRDET = this->numberEntriesOfRDET * 32 / this->bytePerSector;
    this->sectorVolume = Utils::MyINTEGER::Convert2LitleEndian(bootSector.begin() + 0x20, 4);
    this->bytePerSector = Utils::MyINTEGER::Convert2LitleEndian(bootSector.begin() + 0xB, 2);
    this->startSectorOfFAT1 = this->sectorPerBootsector;
    this->startSectorOfRDET = ClusterToSector((int)Utils::MyINTEGER::Convert2LitleEndian(bootSector.begin() + 0x2C, 4));
